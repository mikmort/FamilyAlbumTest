...
module.exports = async function (context, req) {
    ...
    try {
        ...
        // Get a featured person with birthday in current month (or any family member if none found)
        const featuredPersonQuery = `
            WITH BirthdayPeople AS (
                SELECT 
                    ne.ID,
                    ne.neName,
                    ne.neRelation,
                ...
                AND ne.Birthday IS NOT NULL
                AND MONTH(ne.Birthday) = @currentMonth
            ...
            SELECT TOP 1 ID, neName, neRelation, neCount, Birthday
            FROM BirthdayPeople
            ORDER BY Priority, NEWID()
        `;
        
        const featuredPersonResult = await query(featuredPersonQuery, { currentMonth });
        const featuredPerson = featuredPersonResult.length > 0 ? featuredPersonResult[0] : null;

        // Get a featured event that occurred in current month (or any event if none found)
        const featuredEventQuery = `
            WITH MonthEvents AS (
                ...
                AND MONTH(ne.EventDate) = @currentMonth
                AND EXISTS (SELECT 1 FROM dbo.NamePhoto np WHERE np.npID = ne.ID)
            ...
            SELECT TOP 1 ID, neName, neRelation, neCount, EventDate
            FROM MonthEvents
            ORDER BY Priority, NEWID()
        `;
        
        const featuredEventResult = await query(featuredEventQuery, { currentMonth });
        const featuredEvent = featuredEventResult.length > 0 ? featuredEventResult[0] : null;

        // Get a random suggestion (any event with photos)
