/**
 * Find and convert all MOV files to MP4
 * This script:
 * 1. Queries database for all MOV files
...
async function main() {
  console.log('🔍 Finding MOV files in database...\n');
  ...
  try {
    ...
    const movFiles = result.recordset;
    console.log(`Found ${movFiles.length} MOV files to convert\n`);

    if (movFiles.length === 0) {
      console.log('✓ No MOV files found. All done!');
      return;
    }
    ...
    for (let i = 0; i < movFiles.length; i++) {
      ...
      const mp4FileName = fileName.replace(/\.MOV$/i, '.mp4');

      console.log(`[${i + 1}/${movFiles.length}] Processing: ${fileName}`);

      try {
        // 1. Download MOV from blob storage
        console.log('  ↓ Downloading...');
        const movBlobClient = containerClient.getBlobClient(fileName);
        const movPath = path.join(tempDir, path.basename(fileName));
        const mp4Path = path.join(tempDir, path.basename(mp4FileName));

        await movBlobClient.downloadToFile(movPath);
        console.log(`  ✓ Downloaded (${(fs.statSync(movPath).size / 1024 / 1024).toFixed(2)} MB)`);

        // 2. Convert to MP4 using FFmpeg
        console.log('  🎬 Converting to MP4...');
        const ffmpegCmd = `ffmpeg -i "${movPath}" -c:v libx264 -preset medium -crf 23 -c:a aac -b:a 128k -movflags +faststart -y "${mp4Path}"`;
        ...
        // We need to temporarily disable the constraint or update both in a transaction
        ...
        // 5. Delete original MOV from blob storage
        console.log('  🗑️  Deleting original MOV...');
        await movBlobClient.delete();
    ...
    console.log(`  📁 Total: ${movFiles.length}`);
